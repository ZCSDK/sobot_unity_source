...
@param isOnlyShowTicket YES 只显示留言记录界面，不能创建工单，NO 可以提交留言和查看留言记录,  默认NO
@param byController 当前执行跳转的VC           not null
@param CloseBlock 留言点击返回时间 code = 0 ,正常关闭， ，其他情况发送错误 如获取不到模版
 
 注意从430的版本开始，UI改版界面跳转反生变化，留言和和留言记录是一个主界面流转流程，都会先调整到留言模块主页面
*/
+ (void)openLeaveisOnlyShowRecored:(BOOL) onlyShowRecored Kitinfo:(ZCKitInfo *)kitInfo with:(UIViewController *)byController onItemClick:(void (^)(NSString *msg,int code))CloseBlock;
...
///      time  收到最后一条消息的时间戳 （未读消息、离线消息、未确认消息 三者比较取时间为最后的一条消息）** 注意从4.4.0版本开始废弃 **
+(void)getOfflineMsgAndUnAckMsgWith:(NSString *)partnerid appkey:(NSString*)appkey resultBlock:(void (^)(NSDictionary *object, NSString *message,int unReadSize,int offlineSize,int unAckSize))ResultBlock;
