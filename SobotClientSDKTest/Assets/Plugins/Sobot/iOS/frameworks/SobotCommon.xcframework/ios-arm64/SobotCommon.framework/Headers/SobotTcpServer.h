...
#import <SobotCommon/SobotSocketConnect.h>
...
/// 检查消息类型,301/205/0 等无用消息,同时做 msgId 去重(_repeatDict 加锁)。
/// 内部会调 confirmMsg:,该方法已根据 self.connected 判断 IM 是否在线;
/// 轮询路径(IM 已断)调用此方法时,内部 ack 自动 no-op,业务层另走 HTTP /ack。
/// @param dict  消息体
