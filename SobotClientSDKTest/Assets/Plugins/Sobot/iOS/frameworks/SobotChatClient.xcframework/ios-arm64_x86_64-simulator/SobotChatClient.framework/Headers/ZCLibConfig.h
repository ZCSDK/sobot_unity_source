...
#import <SobotChatClient/ZCLibCusMenu.h>
...
// 4.6.3 新增 暂无客服接待的昵称
// 无人接待昵称开关。0=关闭，1=开启。null 时按 0 处理
@property(nonatomic,copy)NSString *topBarNoStaffNickname;
// 无人接待昵称内容
// 当 topBarNoStaffShowFlag=1（无人接待显示开启）且 topBarNoStaffNicknameFlag=1 时 显示
@property(nonatomic,assign)BOOL topBarNoStaffNicknameFlag;
...
* 导航条颜色模式：
*  0=无颜色：1=单色 2=渐变色
     注意 * topBarBackStyle = 0  rebotThemeStyle = 2 ，的场景下 导航栏的背景色要随系统渐变，文字和按钮也要自己处理暗夜和白色的适配
*/
...
// 关 1开 对应大模型转人工按钮配置,==1添加转人工按钮
@property(nonatomic,assign) int aiTransferButtonFlag;


// V7 当前是pending pending ：true 表示在待分配会话池里，需要展示待分配说辞和隐藏转人工按钮
@property(nonatomic,assign) BOOL pending;

// V7 待分配池 返回的富文本的人工欢迎语
@property(nonatomic,strong)NSMutableArray *adminHelloWordRichMessage;

// V7 新增待定池方案  //接待模式 0-实时，1-异步
@property(nonatomic,assign) int assignmentMode;
// "allocatedWord":"请稍等～我将在晚点的时间回复您。" // 进入待分配列表说辞
@property(nonatomic,copy) NSString *allocatedWord;
// //富文本格式进入待分配说辞[]
@property(nonatomic,strong) NSMutableArray *allocatedWordRichMessage;
// 进入待分配列表说辞开关 0关 1开启 「配合转人工status:10使用」
@property(nonatomic,assign) int allocatedFlag;

// 进入待分配列表说辞开关 0关 1开启 「配合转人工status:10使用」

// 人工是否有上传头像  是否是默认头像，0-不是 1-是
@property(nonatomic,assign) int isDefaultFace;

// ","topBarFontIconColorStyle":1,"topBarFontIconColor":""}
@property(nonatomic,copy)NSString *topBarFontIconColorStyle;

/**
* 导航条背景上边的文字图标颜色模式：0：白色：1：深色(对比后的结果，具体颜色用本地)
* 通过topBarFontIconColorStyle判断后再使用  现在返回颜色值
*/
@property(nonatomic,copy)NSString *topBarFontIconColor;

/**
     * 聊天页背景图开关：0、关闭：1、开启：默认关闭
     */
@property(nonatomic,assign) int chatBackPhotoFlag;
    /**
     * 聊天页背景图url地址
     */
@property (nonatomic , strong) NSString *chatBackPhotoUrl;
 ...
 *  -1.机器人
 *  0.离线
 *  1.在线     //仅人工且在待分配会话池内，会返回1    如何区分是仅人工也是转人工成功的场景？？？
 */
...
// 导航条接待状态 样式1 客服
@property(nonatomic,assign) int topBarStaffDescribeFlag;

// 导航条显示状态文案 样式1 客服
@property(nonatomic,copy) NSString *topBarStaffDescribe;

// 导航条接待状态 样式1 客服  无人接待显示开关
@property(nonatomic,assign) int topBarNoStaffShowFlag;

// 导航条接待状态 样式1 客服  无人接待头像开关
@property(nonatomic,assign) int topBarNoStaffPhotoFlag;

// 导航条显示状态文案 样式1 客服  无人接待头像链接
@property(nonatomic,copy) NSString *topbarNoStaffUrl;

// 导航条接待状态 样式1 客服  无人接待描述开关
@property(nonatomic,assign) int topBarNoStaffDescribeFlag;

// 导航条显示状态文案 样式1 客服  无人接待描述内容
@property(nonatomic,copy) NSString *topBarNoStaffDescribe;
...
// 450 新增变量收集
@property(nonatomic,assign) BOOL checkFormSubmitOver;
// 450 新增 表单收集信息
